...
# short grace to exit, then escalates to SIGKILL. The SIGTERM/SIGKILL SIGKILL
...
def test_kill_process_tree_escalates_to_sigkill_when_mount_survives_term():
    ...
    from factory.orchestrator.pi_backend import _SIG_KILL, _SIG_TERM
...
def test_kill_process_tree_winds_down_after_term_without_sigkill():
    if sys.platform == "win32":
        pytest.skip("killpg semantics are POSIX-only")
    from factory.orchestrator.pi_backend import _SIG_TERM
...
def test_run_launches_child_as_group_leader_on_posix(monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.setattr("factory.orchestrator.pi_backend._DEFAULT_IDLE_TIMEOUT_S", 1.0)
    monkeypatch.setattr("factory.orchestrator.pi_backend._DEFAULT_TOTAL_TIMEOUT_S", 1.0)
...
def test_kill_process_tree_terminates_real_grandchild(monkeypatch, tmp_path) -> None:
    ...
    from factory.orchestrator.pi_backend import _SIG_KILL
...
def test_run_retries_transient_spawn_failure_then_succeeds(monkeypatch, tmp_path) -> None:
    from factory.orchestrator.pi_backend import _SPAWN_RETRIES
...
def test_retry_gives_up_after_budget_when_spawn_always_fails(monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.setattr(
        "factory.orchestrator.pi_backend._SPAWN_BACKOFF_S", 0.0
...
def test_run_caps_output_and_keeps_final_manifest(monkeypatch, tmp_path) -> None:
    monkeypatch.setattr(
        "factory.orchestrator.pi_backend._MAX_OUTPUT_TOTAL_CHARS", 800
    )
    monkeypatch.setattr(
        "factory.orchestrator.pi_backend._MAX_OUTPUT_RETAINED_CHARS", 400
    )
...
def test_run_output_under_cap_has_no_truncation_note(monkeypatch, tmp_path) -> None:
    monkeypatch.setattr(
        "factory.orchestrator.pi_backend._MAX_OUTPUT_TOTAL_CHARS", 1000
    )
    monkeypatch.setattr(
        "factory.orchestrator.pi_backend._MAX_OUTPUT_RETAINED_CHARS", 500
    )
...
def test_retain_line_capped_truncates_a_lone_oversized_line() -> None:
    ...
    from factory.orchestrator.pi_backend import _retain_line_capped
...
def test_kill_process_tree_post_sigkill_poll_is_bounded(monkeypatch) -> None:
    ...
    from factory.orchestrator.pi_backend import _kill_process_tree
...
def test_kill_process_tree_default_probe_escalates_for_a_running_child(
    ...
    monkeypatch.setattr(
        "factory.orchestrator.pi_backend._pid_is_alive",
        lambda pid: True,
    )
    _kill_process_tree(
        _RunningProc(),
