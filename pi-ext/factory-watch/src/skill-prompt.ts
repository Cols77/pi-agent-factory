...
export function buildPlanSeedPrompt(topic: string, skillBlocks: string[]): string {
  const instructions = [
    ...
    "At the start, persist the user's initial request verbatim in `.factory/planning/<run-id>/intent.json` as schema 2 through the deterministic capture boundary. Record every clarification verbatim with its question, source/provenance, sequence, and stable unique id; do not replace or paraphrase the original prompt.",
    ...
    "After a clean result, display the stable downstream menu: `standard-development`, `health-recovery`, or `feature-planning`. The menu is informational and no choice launches a process. To materialize a selected bridge, invoke `/plan-handoff <run-id> <workflow>` explicitly; a new session must revalidate the canonical handoff and source hashes before acting.",
    "This planning workflow only proposes downstream work. Never start FEAT-13, `/factory-run`, or any development process automatically from plan-time; browser projection is optional and not required.",
  ].join("\n\n");
