...
from coherence.simulation.registry import Run, load_runs, runs_for
from coherence.goals.evaluator import GoalResult, evaluate
from coherence.goals.lifecycle import can_transition
from coherence.goals.registry import record
...
def evaluate_goals_from_runs(evidence_dir: Path, goals) -> list:
    ...
    metrics.json, call ``coherence.goals.evaluator.evaluate``, then persist via
    ``coherence.goals.registry.record``. ``goals`` is the dict from
    ``coherence.goals.registry.load_goals``. Returns the list of GoalResult in goal
...
def evaluate_goal_from_runs(evidence_dir: Path, goals, goal_id: str) -> dict:
    ...
    ``goals`` is the ``coherence.goals.registry.load_goals`` dict. Returns a
