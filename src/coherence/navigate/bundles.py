...
# `adr`/`feat`/`metric`/`goal`/`contract` refs are id-based, matching
# `sr:`/`task:` and unlike `spec:`/`plan:`, which are repo-relative paths.
# Resolution to a file happens in the caller, never here -- this only parses
# the ref. `contract:` is admitted additively; its resolution to the artifact's
# real path goes through the compiled contract catalog in coverage, never by
# re-parsing catalog YAML here.
_MEMBER_KINDS = (
    "spec",
    "plan",
    "task",
...
def _parse_member_ref(raw_ref: str) -> SystemScopeRef | None:
    ...
    `task:`/`sr:`/`adr:`/`feat:`/`metric:`/`goal:`/`contract:` prefix and a
    non-empty identifier after it. Anything else does not resolve (design §3.3)
    and is reported `missing` by the caller rather than raised, so one bad
    member never drops the whole bundle.
    """
