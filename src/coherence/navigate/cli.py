"""`python -m coherence.navigate` CLI: `brief`, `matrix`, `timeline`, `story`,
...
from coherence.simulation.evidence import evaluate_goal_from_runs
...
from coherence.navigate import health as health_module
from coherence.navigate import labels as labels_module
from coherence.navigate import remediation as remediation_module
from coherence.navigate import vocabulary as vocabulary_module
from coherence.navigate.bundles import list_bundles
from coherence.navigate.coverage import bundle_coverage, member_target
from coherence.navigate.guide import export_guide
...
def _guard_scope(repo_root: Path, scope_raw: str):
    scope = parse_scope_ref(scope_raw)
    snapshot = resolve_navigation_snapshot(repo_root, scope.ref)
    return scope, snapshot


def _stale_scope_result(scope, snapshot) -> dict:
    return {
        "scope": to_dict(scope),
        "stale": True,
        "freshness": snapshot.freshness,
        "snapshot": {"ref": snapshot.ref},
        "resolver": snapshot.resolver_cmd,
        "message": "navigation input is not current; resolve the snapshot before rendering content",
    }
...
def cmd_sim_metric(repo_root: Path, metric_id: str) -> list[dict]:
...
def cmd_bundle_new(
    repo_root: Path, feature_ref: str, output: str, *, force: bool = False
) -> dict:
    payload = bundles_module.create_draft_bundle(
        repo_root, feature_ref, Path(output), force=force
    )
    return {"bundle": payload, "output": str(Path(output))}
...
def _render_bundle_new(result: dict) -> str:
    bundle = result["bundle"]
    return (
        f"draft bundle: {bundle['id']} -- {bundle['label']}\n"
        f"  members       {len(bundle['members'])}\n"
...
def _render_dossier_bundle(result: dict) -> str:
    """Human render for `coherence.navigate dossier`: one header per section, each
...
def main(argv: list[str] | None = None) -> int:
    ...
    # matching coherence.register.cli's convention.
    ...
    p_bundle_new = bundle_sub.add_parser("new", parents=[common])
    p_bundle_new.add_argument("--from", dest="feature_ref", required=True)
    p_bundle_new.add_argument("--output", required=True)
    p_bundle_new.add_argument("--force", action="store_true")
    ...
    p_membership = sub.add_parser("membership", parents=[common])
    p_membership.add_argument("--gate", action="store_true")
    p_membership.add_argument("--force", action="store_true")
    ...
    if args.cmd == "worker":
        from coherence.navigate.worker import run_worker
    ...
    try:
        if args.cmd == "brief":
            ...
            if args.bundle_cmd == "new":
                result = cmd_bundle_new(
                    args.repo_root, args.feature_ref, args.output, force=args.force
                )
                rendered = _render_bundle_new(result)
            else:
            ...
            from coherence.navigate.dossier import cmd_dossier as _cmd_dossier
    ...
    if args.cmd in {"coverage", "membership"} and args.gate and result["unbundled"]:
        if args.force:
