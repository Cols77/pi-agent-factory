...
-> `satisfies` edges mapped through `coherence.navigate.refs.
...
- `substrate.ledger.tasks` for the task record a matched run's
  ...
  field `coherence.trace.model.extract_edges` reads to build the task's
...
def _resolve_scope_file(repo_root: Path, scope: SystemScopeRef) -> Path:
    ...
    # Deferred import: coherence.navigate.guide imports query_brief/query_matrix/
    # query_timeline from coherence.navigate.queries at its own module level, so
