...
- `coherence.navigate.bundles` for declared feature-scope bundles (syntactic
  member parsing; Task 1's job);
- `coherence.register.register` for SR content and binding, via the
...
- `coherence.trace.validation_status` for validation report outcomes and
  staleness;
...
from coherence.goals import registry as goal_registry
...
from coherence.navigate.bundles import BundleIdMismatchError
from coherence.navigate.coverage import ArtifactLookup, build_artifact_lookup
...
# Member kinds a declared bundle may name (mirrors coherence.navigate.bundles).
...
def query_goal(repo_root: Path, goal_id: str) -> dict:
    ...
    goal file's sole parser is `coherence.goals.schema.parse_goal`. A goal id
...
def query_goal_evidence(repo_root: Path, goal_id: str) -> dict:
    ...
    Wraps ``coherence.simulation.evidence.evidence_for_goal`` so the navigator
...
def _resolve_spec_or_plan_member(repo_root: Path, member: SystemScopeRef, identifier: str) -> _MemberResolution:
    ...
    # (function-local) because `coherence.navigate.guide` imports several
    # `query_*` functions from this module at its own module level -- a
    # module-level import here would be circular.
    from coherence.navigate import guide as _guide
...
def _resolve_adr_member(
    ...
    `coherence.navigate.adr` rather than the trace loader.
...
def _document_titles(repo_root: Path) -> dict[Path, str]:
    ...
    Reuses `coherence.trace.model.load_nodes` -- the loader that already derives
...
def _task_implementation_summary(repo_root: Path, task_id: str) -> tuple[dict, SystemClaim | None]:
    ...
    because `coherence.navigate.story` imports `ScopeKindError`/
    ...
    *latest* run only (`query_story`'s own oldest-first ordering,
    ...
    """
    from coherence.navigate.story import query_story  # deferred: story.py imports this module
...
def _load_validation_statuses(repo_root: Path, report_corrupt: bool) -> dict[str, SrStatus]:
    ...
    `coherence.trace` (`raw.get("requirements", [])` on a non-dict raises
    ...
    report, instead of ever reaching `coherence.trace` with a shape it cannot
    handle.
    """
    if report_corrupt:
        return {}
...
#   - `validation/validation-report.json` entries (`coherence.trace.
...
def query_guide(repo_root: Path, scope: SystemScopeRef) -> dict:
    ...
    All synthesis logic lives in `coherence.navigate.guide` -- this only
    ...
    not module-level) because `coherence.navigate.guide` itself imports
...
def _traversal_for_sr(
    ...
    Walks the same `coherence.trace.model.extract_edges` edges `build_graph`
    ...
    - `files`: changed files recorded in the evidence manifests of the
      ...
      `coherence.navigate.reverse`/`story` read (task -> run -> files).
...
def query_traversal(repo_root: Path, scope: SystemScopeRef) -> dict:
    ...
    from coherence.navigate.labels import build_alias_map
...
def query_feature_context(repo_root: Path, scope: SystemScopeRef) -> dict:
    ...
    from coherence.navigate.feature import feature_context
...
def query_validation(repo_root: Path, scope: SystemScopeRef) -> dict:
    ...
    * the metric ids the bound goals evaluate.
    ...
    from coherence.register import register as req_register
...
def query_vcycle(repo_root: Path, scope: SystemScopeRef) -> dict:
    ...
    from coherence.navigate.vcycle import vcycle_slice
