...
from factory.presentation import browser as browser_module
from factory.presentation import ide as ide_module
from factory.presentation import sim as sim_module
from factory.presentation.level import Facts, Level, decide
from factory.system.queries import ScopeKindError, parse_scope_ref
...
def _resolve_scope(repo_root: Path, artifact: str, focus: str | None, level: Level) -> ResolvedIntent:
    ...
    if kind == "file":
        ...
        if abs_path is None:
            return ResolvedIntent(artifact, focus, level, None, None, reason)
...
def resolve_intent(
    ...
    *,
    ...
    if abs_path is None:
        return ResolvedIntent(artifact, focus, chosen, None, None, reason)
    line = _focus_line(focus)
    target = ide_module.build_ide_uri(abs_path, line)
    line_txt = f" line {line}" if line else ""
