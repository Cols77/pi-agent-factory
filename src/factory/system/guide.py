...
a small closed set) `factory.system.queries` already computed -- the same
...
`is_exported_guide` lets `factory.system.queries` refuse to cite one as a
...
def _aggregate_freshness(freshnesses: list[Freshness]) -> Freshness:
    ...
    Never recomputes staleness -- only rolls up states `factory.system.
...
def query_guide(repo_root: Path, scope: SystemScopeRef) -> dict:
    ...
    built entirely from `factory.system.queries.query_brief`/`query_matrix`/
...
def is_exported_guide(path: Path) -> bool:
    ...
    Used by `factory.system.queries._resolve_spec_or_plan_member` to refuse
