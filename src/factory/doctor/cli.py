...
from factory.doctor.write import emit_task, mint, promote
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.cmd == "context":
            ...
            path = mint(
                args.project_root, args.source, args.title, args.statement, args.domain
            )
        ...
        # Refusals (unknown requirement, missing source, empty dod) are the
        # caller's problem to fix, not a traceback to read.
        print(str(exc))
        return 1
    return 0
