...
| Projection composition | **`factory.system.health.query_health(root)`** composes `compute_health`, `bundle_coverage`, `ordered_bundle_ids`, and `bundle_readiness`. Readiness derives each SR signal from `register`/`gaps`/`validation_status` directly (no `classify` call — `classify` is the requirements-closure vocabulary, not a readiness input). It **duplicates no logic** — it re-shapes existing results. | SP-A→SP-B interface, and the decomposition's "composes, never re-derives". |
