...
**Status:** design
**Date:** 2026-08-18
**Amended:** 2026-08-20 — canonical agentic I/O; see §9.3 and §15.
...
- Evidence absence and evidence emptiness are the same value. A task with no run manifest
  produces `TaskScope(changed_files=(), manifests=())`, which the audit collapses into
  `{"ok": false, "reason": "no changed files from tasks"}` — the identical result to a task
  that ran and touched nothing. `evidence reconcile` *does* detect the absence
  (`MISSING_EVIDENCE`, "completed task has no validated evidence manifest"), but the audit
  never consults it and nothing can resolve it: `repair_reconciliation` deliberately refuses,
  because no deterministic operation can invent absent provenance.

- Agent-facing inputs and outputs have no shared contract. Shell output can be compacted by
  RTK, but tests, simulations, Git state, requirement closure, experiments, plans and code
  navigation each expose unrelated shapes. Some are durable source artifacts, some are
  indexed snapshots, and some are time-bound observations; treating all of them as formatted
  text loses provenance, while treating all of them as one universal result would erase
  domain semantics.
...
134 are unbound, 5 have no satisfier, and exactly one feature has ever been audited — a run
that found its own blocker and misnamed it.

**Correction to `kb-0006` / TN-01.** The original note said run manifests are written with an
empty `changed_files`. That is false, and the plan built on it would have been a no-op: every
one of the drone repo's seven v1 manifests records between 2 and 41 changed files, written by
`evidence/finalize.py` from a real `git diff`. The FEAT-NAV-017 audit came back blind for a
different reason — its two satisfying tasks, `T-058` and `T-067`, are marked `status: done`
and have **no manifest at all**, having been completed outside the orchestrator. The defect
is therefore the conflation described above plus the missing repair path, not the writer.
...
6. One provenance-preserving agentic I/O architecture: durable artifacts and snapshots are
   addressable through stable references; time-bound results use one observation envelope;
   deterministic projections make either form compact for agents without becoming evidence.
7. One freshness compiler/resolver built from the existing fingerprint, reconciliation,
   requirement-check and self-refresh mechanisms: stale use is detected at consumption, pure
   derived state may rebuild automatically, and intent or missing provenance never is invented.
...
| D11 | Canonical agentic I/O has two primitives: artifact/snapshot references and observation envelopes | Code maps, plans and requirements are durable state; tests and simulations are observations. Forcing both into one result schema would make the contract dishonest. |
| D12 | Compact output is a deterministic projection, never the evidence source | RTK demonstrates the value of reducing output before it reaches an agent, but lossy text cannot support gates, freshness or audit. Raw/domain-native artifacts remain available and are content-hashed. |
| D13 | Adopt the contracts in `substrate`, migrate producers incrementally | The boundary affects every Coherence consumer and therefore must be decided before the package carve. Requiring a flag-day artifact rewrite would strand increment 0 and make the refactor unsafe. |
| D14 | Extend the existing freshness machinery into declarative recipes and policy-bound resolvers | The repo already fingerprints dependencies, rebuilds the code index, reconciles evidence and names remediation commands. Centralising their contracts avoids a second engine; classifying resolvers prevents automatic repair from rewriting intent or fabricating provenance. |
...
| `substrate.freshness` | fingerprints, severity model, declarative freshness recipes, compiled dependency graph, resolver protocol and `GATE_FAILING_SEVERITIES` | `factory.freshness` plus adapters over existing local repair paths |
...
| `substrate.artifacts` | `ArtifactRef`/`SnapshotRef`, content hashes, provenance and freshness metadata for code, specs, plans, tasks, requirements and evidence | new; composes existing ledger, trace and freshness models |
| `substrate.observations` | versioned `ObservationEnvelope`, typed payload registration and domain adapters for commands, tests, simulations, experiments, Git and requirement checks | new; adapters wrap existing result models |
| `substrate.projections` | deterministic machine, human and token-budgeted agent views over artifacts, snapshots and observations | new; absorbs no source of truth |
...
### 9.3 Canonical agentic I/O

RTK is the precedent for a projection boundary: execute or read something verbose, then give
the agent a deterministic compact view. Coherence generalises the boundary without making
RTK output authoritative. The detailed contract is specified in
`2026-08-20-coherence-agentic-io-design.md`.

Two primitives are deliberately separate:

- `ArtifactRef`/`SnapshotRef` identify durable inputs and indexed state. Code navigation uses
  the existing fingerprinted `substrate.codemap`; specs, plans, tasks and requirements remain
  authoritative Markdown parsed by `substrate.ledger`, `coherence.trace` and
  `coherence.register`. References carry kind, stable ref, content/freshness fingerprint,
  producer and scope refs. They do not copy every artifact into a new store.
- `ObservationEnvelope` records a time-bound result: command/test outcome, simulation trials,
  experiment metrics, Git state, requirement closure, gate result or audit finding. It carries
  schema version, producer, observed time, input snapshot refs, outcome, typed facts,
  diagnostics and content-hashed output artifact refs.

Adapters translate existing domain models into envelopes. Payloads remain separately typed
and versioned; a Git observation is not made to look like a simulation trial. Projectors then
render machine JSON, a human explanation or an RTK-like compact agent view. Truncation and
redaction are projection concerns and are always visible; they never change the stored result
or allow an unknown/invalid observation to become a pass.

#### Freshness compilation and resolution

This extends mechanisms already present rather than replacing them. `factory.freshness`
provides file, value, tool and Git-tree fingerprints; `codeindex.ensure_fresh` detects and
rebuilds its own derived index; evidence reconciliation detects stale manifest dependencies
and repairs only explicit-provenance cases; requirement checks detect stale bindings; and the
navigator remediation catalogue names commands but deliberately does not execute them.

Each producer declares a `FreshnessRecipe`: output kind, input selectors, fingerprint
function, resolver id, resolution class and cost/side-effect bounds. Recipe compilation
validates unique outputs, known resolvers and an acyclic dependency graph. On consumption, a
guard recomputes the inputs, emits a staleness observation on mismatch, and dispatches only a
resolver permitted by policy. Successful resolution produces a new reference/envelope with
lineage to the stale one and is revalidated before use.

Resolution classes are fixed:

- `derived_auto`: deterministic disposable state such as code, trace and requirement indexes
  or compact projections; rebuild automatically.
- `repeatable_policy`: tests, simulations, audits and other executable evidence; rerun only
  within configured time, cost and side-effect bounds.
- `authoritative_gate`: specs, plans, requirements and deferrals; detect and route to the
  inbox or owning writer, never rewrite automatically.
- `provenance_blocked`: missing historical evidence or unattributed changes; report a blocker
  because no resolver may invent the absent provenance.

Resolution is bounded to one attempt per recipe/input fingerprint in a run, cycle-checked and
recorded as an observation. There is no background daemon and no silent semantic repair:
checks happen on guarded read and at explicit status/preflight sweeps.

The first migration is narrow: gate/test output, measurement results, Git/evidence state and
requirement checks. Code-map and planning artifacts adopt stable snapshot references at the
same seam, but their existing parsers, indexes and source files stay in place. Other producers
move when their owning Coherence increment lands.
...
| # | Contents | Gate |
|---|---|---|
| 0 | TN-01 (corrected): audit distinguishes missing evidence from empty evidence, and `evidence record` makes missing evidence resolvable; TN-11 bootstrap this repo's own register | re-auditing FEAT-NAV-017 names `T-058`/`T-067` as evidence-less rather than reporting no overlap; after `evidence record`, the same audit returns a real overlap; `trace check` runs against a non-empty register |
| 1 | `substrate` extraction (paths, config, schemas, ledger, agents, validators, evidence read model, freshness, codemap, kb) + artifact/snapshot references, observation envelopes, projection contracts and the freshness recipe/resolver protocol + shims; TN-14 signatures reach retrieval | full gate green; no module in `substrate` imports `factory` or `coherence`; contract fixtures round-trip without losing provenance; the compiled recipe graph rejects cycles; the code index rebuilds through a `derived_auto` resolver; a KB entry with an error signature is selected by a failing gate's output |
...
| 3 | `coherence.navigate` + `presentation` + `goals` + `simulation`; TN-06 bundle authoring; TN-10 remediation as tools; `membership` rename; code and planning views consume snapshot refs | navigator serves every route; stale snapshots are identified rather than rendered as current; `membership --gate` matches old `coverage --gate` |
| 4 | `coherence.audit` + `coherence.measurement`; observation adapters for audits, tests, simulations and experiments; TN-09 parallel per-SR audits; TN-13 codemap overlap; remaining TN-08 renames | a multi-SR feature audits concurrently with identical verdicts; compact projection and machine envelope agree on outcome and artifact refs |
| 5 | `coherence status`, `focus`, `explain`; TUI widget; `/using-coherence` + skill; `/factory-doctor` → `/factory-selfcheck`; compact agent projections | dispatcher routes all eight intents; menu ranks correctly against seeded states; projections stay within budget and disclose truncation; the old command name still runs and warns |
| 6 | Gate protocol (absorbs TN-02) + inbox + expiring deferrals + unresolved-staleness routing | no gate can finalise without a decision; every finding source appears in the inbox; authoritative and provenance-blocked stale items name their owning resolver or blocker |
| 7 | Unified long-run surface for factory runs, audits and measurement, backed by observation envelopes | one status protocol, one mission control, completion notification; raw artifacts remain reachable from every projected result |
...
intent. Agentic I/O adds contract fixtures per domain adapter, round-trip tests for artifact
references and observation envelopes, golden compact projections, and invariants that a
projection cannot change an outcome, hide invalidity or claim freshness absent a matching
fingerprint. Freshness tests compile seeded recipe graphs, reject cycles and unknown
resolvers, prove one-attempt bounds, and verify each resolution class: automatic rebuild,
policy-controlled rerun, human gate and provenance blocker.
...
- **A universal envelope erases domain meaning.** Mitigated by D11: only provenance and
  routing are common; payloads remain typed and versioned per domain, while durable artifacts
  use references rather than result envelopes.
- **Compact output is mistaken for evidence.** Mitigated by D12: projections disclose
  truncation/redaction and link to content-hashed raw artifacts; gates consume the machine
  envelope and domain artifacts, never the compact prose.
- **Automatic freshness repair rewrites intent or loops.** Mitigated by D14: only disposable
  derived state is unconditionally automatic; executable evidence is policy-bound;
  authoritative and provenance-blocked cases cannot auto-resolve. Recipe compilation rejects
  dependency cycles and resolution is attempted once per input fingerprint per run.
...
## 15. Amendment record: canonical agentic I/O and freshness

This section records how the 2026-08-20 amendment changes the design committed at `1e31544`
and the corrected TN-01 text present in the working tree. It is evidence for design review,
not a second roadmap.

| Original concept | Amendment | What still lands |
|---|---|---|
| Three layers: `factory` → `coherence` → `substrate` | Adds `substrate.artifacts`, `substrate.observations` and `substrate.projections`; extends `substrate.freshness` with recipes and a resolver protocol; dependency direction is unchanged | The same three-layer carve and compatibility shims |
| `substrate.codemap` unifies signatures and import edges | Code indexes expose fingerprinted `SnapshotRef`s and register their existing self-rebuild as `derived_auto` | TN-13, multi-language overlap and symbol-aware KB scope |
| `substrate.ledger` owns task/plan parsing | Plans, specs, tasks and requirements remain authoritative files; parsed state is addressable through artifact/snapshot refs; staleness routes to owning writers | Existing plan/task semantics, trace edges and source-plan checks |
| Status, inbox and gates read each subsystem directly | Domain adapters produce observation envelopes; guarded reads emit staleness observations and may invoke policy-approved resolvers | One status, one findings inbox and one gate protocol |
| Measurement owns harness-specific `HarnessResult` data | Harness payloads remain domain-specific but gain common provenance, input snapshots and policy-controlled rerun metadata | Simulation, Playwright and experiment semantics unchanged |
| Freshness is implemented locally by fingerprints, checks and repair commands | Declarative recipes compile those mechanisms into one dependency graph and four resolution classes; existing detector/resolver code is adapted, not replaced | Code-index freshness, requirement checksum checks, evidence reconciliation and navigator remediation |
| Unified long-run surface is a UI/protocol convergence | Observation envelopes become its shared result protocol; projections supply bounded agent/human views; resolver activity appears in the same surface | Mission control, resumability and completion notification |
| RTK compacts shell output outside the factory | Coherence adopts the pattern, not a hard RTK dependency; raw capture precedes lossy projection | Existing shell workflows continue to use RTK where available |

Nothing from increments 0–8 is removed. Increment 0 remains first and unchanged by this
amendment. Increment 1 gains the shared contracts and adapts the existing code-index refresh
