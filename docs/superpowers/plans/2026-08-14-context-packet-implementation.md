...
^- [x] **Step 1 (failing tests):** `primary_paths(task, manifest)` returns the
...
^- [x] **Step 2 (implement):** stdlib `ast`/`tokenize` signature extractor
...
^- [x] **Step 1 (failing tests):** after a passing context-gather, the run persists
...
^- [x] **Step 2 (implement):** add `packet` param to `compose_prompt` and thread it
...
^- [x] **Step 3:** unit suite + integration smoke + ruff; commit.
...
^- [x] **Step 1 (failing tests):** a new `readContextPacket(cwd, sessionId)`
  ...
  task-text-only.
^- [x] **Step 2 (implement):** read + inject a bounded `render_packet` slice into the
  grill seed; keep the existing explicit-explainers path.
^- [x] **Step 3:** vitest + typecheck + full guard suite; commit.

### Task 4: Review handoff

^- [x] **Step 1:** reviewer sub-agent — verify (a) Dev prompt carries primary content;
