...
- [x] **Step 1: Failing tests** — given a `query_feature_context` JSON fixture, the widget renders
...
- [x] **Step 2: Implement** `system-feature-view.ts` as a pure data→DOM function (testable without
  the docs server), reusing the existing claim-rendering helpers. Register `tabFeature` in the tab strip.
- [x] **Step 3:** TS vitest + `uv run python -m pytest -q` + lint green; commit.

## Task 2: Interactive V-cycle tab

- [x] **Step 1: Failing tests** — a `query_vcycle` fixture with a definition side
...
- [x] **Step 2: Implement** `system-vcycle-view.ts` reusing SP-B's graph-layout/click affordances;
  register `tabVcycle`.
- [x] **Step 3:** tests + full suites + lint; commit.

## Task 3: Goal / metric status tab

- [x] **Step 1: Failing tests** — a goal fixture renders requirement, metric, target, current value,
...
- [x] **Step 2: Implement** `system-goal-view.ts`; register `tabGoal`. Wire `eng_get_goal` (Inc 4)
  data through the same payload.
- [x] **Step 3:** tests + full suites + lint; commit.

## Task 4: Validation evidence tab

- [x] **Step 1: Failing tests** — a requirement with goal-aware status (`VALIDATED`/
...
- [x] **Step 2: Implement** `system-validation-view.ts`; register `tabValidation`.
- [x] **Step 3:** tests + full suites + lint; commit.

## Task 5: Simulation-run summaries tab

- [x] **Step 1: Failing tests** — a run bundle renders spec §20 fields (experiment, feature,
...
- [x] **Step 2: Implement** `system-sim-view.ts`; register `tabSim`. `eng_present` (Inc 5) routes
...
- [x] **Step 1: Failing tests** — a `diag:` payload (stub + `diagram_file` + `focus` + `illustrates`)
...
- [x] **Step 2: Implement** — `system-diagram-view.ts` as a pure data→DOM function that embeds the
...
- [x] **Step 3: Comprehension entry (D8)** — from a dossier/V-cycle/`/catchup` view, offer an
...
- [x] **Step 4:** tests + full suites + lint; commit.

## Task 6: Navigation integration (spec AC-02, AC-09)

- [x] **Step 1:** from a requirement in the V-cycle view, navigate to parent/child requirements,
...
- [x] **Step 2:** exercise the feature dossier as a hub: dossier → goals → goal evidence → run → sim.
- [x] **Step 3:** integration tests drive the full navigation against seeded real data; existing tab
  behaviour unchanged.

## Task 7: Review handoff

- [x] **Step 1:** reviewer sub-agent (inline review this session; tool broken) — compliance vs spec §7–§10, §9.1–§9.3, §20–§21, AC-02/AC-09,
