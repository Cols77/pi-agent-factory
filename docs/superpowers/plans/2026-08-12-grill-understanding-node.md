...
- [ ] **Step 1 (failing tests):** `FileGrillGate(transcript_dir, repo_root, poll_interval)` polls `<transcript_dir>/grill-result.json` (no `--auto` side effects; a stale-but-readable file unblocks; a reader test round-trips `GrillResult`). A `FakeGrillGate` scripts `[agreed, not-agreed, skipped]`.
- [ ] **Step 2 (implement):** mirror `FileHumanReviewGate`:
  ```python
  @dataclass
  class GrillResult:
  ...
  ```
- [ ] **Step 3:** full `uv run python -m pytest` + lint; commit.
...
- [ ] **Step 1 (failing tests):** with `human_review is not None` and a `FakeGrillGate([agreed])`, `run_task` runs the grill after `context-gather`, before `dev`, and proceeds; `[skipped]` and `[not-agreed]` both proceed to `dev`; with `human_review is None` (`--auto`) the grill node is **not** invoked.
- [ ] **Step 2 (implement):** in `run_task`, after the context-gatherer returns a usable manifest and before the dev inner loop:
  ```python
...
- [ ] **Step 1 (failing tests):** `buildGrillSeedPrompt(taskText, skillBlocks, freshExplainerSummary, grillResultPath)` renders skill blocks + deterministic instructions; handler unit-test asserts a `[Grill now]/[Skip]` select is offered at `grill:blocked` and `[Skip]` writes `grill-result.json` `{decision:"skipped"}`.
- [ ] **Step 2 (implement):**
...
- [ ] **Step 3:** TS vitest + Python suite + lint; commit.
...
- [ ] **Step 1 (failing tests):** when `grill.decision == "not-agreed"`, `write_review_guide` includes `guide["grill"] = {"verdict":"not-agreed","summary":...}`; extension banner renders the pairing warning when present.
- [ ] **Step 2 (implement):** extend the `guide` dict in `run_task` (Task 3 already threads `grill` here); add the `grill` field to `ReviewGuide` in `review-guide.ts` and render the warning in the review banner.
- [ ] **Step 3:** suite + lint; commit.

### Task 6: Review handoff

- [ ] **Step 1:** reviewer sub-agent — verify: (a) never a hard block; (b) `--auto` skips the grill; (c) reuse of existing fresh explainers is deterministic and stale explainers fall back to generate; (d) additive, no collision with in-progress `/visual-explain` hunks; (e) no parallel staleness engine.
